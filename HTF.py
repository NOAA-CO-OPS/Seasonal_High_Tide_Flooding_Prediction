...
class HTF_model:
    ...
    '''
    def __init__(self,station,years_fit,years_assess,years_pred,
        ...
        ------
        ValueError
            Error if one of the inputs is not allowed.
    ...
    def train(self):
        ...
        data_api,data_nonapi = self.pull_data(self.station,self.years_fit,'gmt')
        print('Training the model...')
        self.out_train = self.calc_resids_and_dists(data_api,data_nonapi,bin_val=self.prctile_bin_val)
    ...
    def assess(self):
        ...
        if self.assess_method == 'DusekEtAl':
            ...
            self.out_assess = self.run(self.out_train['data_api'],
                                       self.out_train['data_nonapi'],
                                       self.out_train) 
            # Do the skill assessment #
            print('Calclating model skill on the same observations...')
            self.skill = self.calc_skill(self.out_train['data_api'],
                                         self.out_train['data_nonapi'],
                                         self.out_assess,
                                         self.assess_metric)
            ...
            data_api,data_nonapi = self.pull_data(self.station,self.years_assess,'lst_ldt')           
            # Run the trained model on the new data #
            print('Running the trained model on the new observations...')
            self.out_assess = self.run(data_api,data_nonapi,self.out_train)                                                  
            # Do the skill assessment #
            print('Calclating model skill on the new observations...')
            self.skill = self.calc_skill(data_api,data_nonapi,self.out_assess,self.assess_metric)
    ...
    def predict(self):
        ...
        print('Downloading and formatting the new out of sample observations. This can take a while...')
        data_api,data_nonapi = self.pull_data(self.station,self.years_pred,'lst_ldt')           
        
        # Run the trained model on the new data #
        print('Running the trained model on the new observations...')
        self.out_predict = self.run(data_api,data_nonapi,self.out_train) 

           
    @staticmethod    
    def pull_data(station,years,time_zone):
        '''
        ...
        '''
        data_api = get_API_data(station, str(years[0]), str(years[1]),
                            product=['hourly_height','predictions'],
                            time_zone=time_zone).run()
        data_nonapi = utils.get_nonAPI_data(station)
        return data_api,data_nonapi
    
    @staticmethod
    def calc_resids_and_dists(data_api,data_nonapi,bin_val='pred_adj'):
        # Add the sea level trend to the predictions #
        pred = data_api['predictions']
        pred_adj,adj = utils.add_trend(data_api['predictions'],
                                      data_nonapi['MSL Trend (mm/yr)'],
                                      data_nonapi['Epoch center'])
        
        # Calculate the residuals relative to the adjusted predictions #
        resids = data_api['hourly_height']['val']-pred_adj['val']
        resids = pd.DataFrame({'time':data_api['hourly_height']['time'],
                                   'val':resids})
        
        # Calculate the residual distribution parameters #
        resid_dists = utils.residual_distributions()
        ...
        damped_per = utils.damped_persistence(dists_time['anom_mu'])
        
        out_train = {'data_api':data_api,
                     'data_nonapi':data_nonapi,
                     'pred_adj':pred_adj,
                     'resids':resids,
    ...
    @staticmethod
    def run(data_api,data_nonapi,out_train):
        
        # Get the years and months on which to run the model #
        yrmo = data_api['hourly_height']['time'].dt.to_period('M').unique()
        yru = np.unique(yrmo.year)
        
        for yr in yru:
            # Get the observations for this year #
            data_api_yr = data_api.copy()
            for k in data_api_yr.keys():
                data_api_yr[k] = data_api_yr[k][data_api_yr[k]['time'].dt.year==yr]
            ...
            mu,sigma = utils.calc_dist_params(out_train['dists_time'],
                                              out_train['dists_tide'],
                                              persistence_apply,
                                              px)
            cy = utils.calc_cdf(px,
                                mu,
                                sigma)

                    
            # Add the sea level trend to the predictions #
            pred_adj,adj = utils.add_trend(data_api_yr['predictions'],
                                          data_nonapi['MSL Trend (mm/yr)'],
                                          data_nonapi['Epoch center'])
            
            
            # Calculate hourly freeboard from flood thresh to adjusted predictions #
            freeboard = utils.calc_freeboard(data_nonapi['Derived Minor'].values,
                                             pred_adj)
           
            # Apply cdfs to determine probability that the NTR is >freeboard
            # for each hourly observation #
            prob_hourly = utils.calc_hourly_prob(cy,
                                                 px,
                                                 data_api_yr,
                                                 pred_adj,
                                                 freeboard,
            ...
            prob_daily = utils.calc_daily_prob(prob_hourly,
                                               freeboard,
                                               out_train['resids'])
    ...
    @staticmethod
    def calc_skill(data_api,data_nonapi,out_run,assess_metric):            
        if assess_metric == 'htf_days':
            observed_floods = utils.calc_observed_floods(data_api,data_nonapi)
            bs,bss,bs_se,bss_se = utils.brier_stats(observed_floods,
                                                  out_run['prob_daily']).run()
            reliability,resolution = utils.calc_reliability(observed_floods,
                                                            out_run['prob_daily'])
            accuracy,precision,recall,false_alarm,F1 = utils.calc_confusion_stats(observed_floods,
                                                        out_run['prob_daily'],
                                                        min_prob=0.05)
            skill = {'bs':bs,
                     'bss':bss,
...
class utils:
    ...
    @staticmethod
    def calc_hourly_prob(cy,px,data_api,pred_adj,freeboard,dists_tide):
        '''
        Apply cdfs to determine probability that the NTR is >freeboard
        for each hourly observation 
        '''
        ...
        freeboard['prctile bin'] = pd.cut(pred_adj['val'],
                                         bins=pctile_bins,
                                         labels=False)
        freeboard['month bin'] = pd.cut(data_api['hourly_height']['time'].dt.month,
                                         bins=np.arange(1,14),
    ...
    @staticmethod
    def calc_observed_floods(data_api,data_nonapi):
        yrmodays = data_api['hourly_height']['time'].dt.normalize()
        yn = []
        for d in yrmodays.unique():
            iThisDay = yrmodays==d
            freeboard_day = data_nonapi['Derived Minor'].values - data_api['hourly_height']['val'][iThisDay]
